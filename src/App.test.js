...
test("App fetches shows episodes data from the api and renders it", async () => {
  //Arrange
  mockFetchShow.mockResolvedValueOnce(shows);
  const { getByText, queryAllByText, queryAllByTestId } = render(<App />);

  //Act || Assert

  await wait();
  expect(queryAllByText(/Stranger Things/i)).toBeTruthy();
